...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(Baseurl + "/api/v1/Vendor/login", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const { accessToken, refreshToken, vendor } = data.message;
        ...
        Cookies.set("vendorId", vendor._id);

        // Redirect to the dashboard or another page
        navigate("/");
      } else {
        // Handle errors
        setError(data.data || "An error occurred. Please try again.");
      }
    } catch (error) {
